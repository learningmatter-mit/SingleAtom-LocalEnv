...
def plot_scatter(
    ...
    with plt.style.context(style):
        fig, ax = plt.subplots()
        if num_col ==4:
            ax.scatter(new_targ, new_pred, facecolors=facecolor, edgecolors=edgecolor, alpha=0.75, linewidth=0.5, s=12)
        else:
        ...
        #     set_size(num_col=2, ax=ax)
        #     print(fig.get_size_inches())
        #     fig.set_size_inches(fig.get_size_inches()*0.625)
        #     print(fig.get_size_inches())
        ...
        if num_col==4:
            ax.set_ylabel("Predicted %s (%s)" % (PROPERTIES[prop_key], UNITS[prop_key]), fontsize=7, labelpad=3)
            ax.set_xlabel("Calculated %s (%s)" % (PROPERTIES[prop_key], UNITS[prop_key]), fontsize=7, labelpad=3)
            ax.annotate(
...
def plot_violin(
    ...
    for Z in Z_range:
        ...
        hues.append(["calculated" for i in range(len(dictionary[Z]))])

        ys.append(pred_dictionary[Z])
        Zs.append([Z for i in range(len(pred_dictionary[Z]))])
        hues.append(["predicted" for i in range(len(pred_dictionary[Z]))])
    ...
    my_pal = {"calculated": color1, "predicted": color2}
    with plt.style.context(style):
        f = plt.figure()
        ax = sns.violinplot(
            x="Z",
        ...
        if num_col == 4:
            ax.set_ylabel("%s (%s)" % (PROPERTIES[prop_key], UNITS[prop_key]), fontsize=7, labelpad=3)
        else:
