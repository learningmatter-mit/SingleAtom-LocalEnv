...
class PerSiteDataset(TorchDataset):
    ...
    def __init__(
        ...
        if os.path.exists(dataset_cache):
            ...
            for idx in tqdm(range(len(samples)), position=0, leave=True):
                id_, nxyz, lattice, site_prop, mask = self.compute_prop(*samples[idx])
    ...
    def compute_prop(self, id_, crystal):
        ...
        if self.site_mask:
            ...
            mask_bin = None
            mask = np.asarray(mask_bin)

        return id_, nxyz, lattice, target, mask
...
class TotalDataset(TorchDataset):
    """Dataset to deal with NFF calculations.
    ...
    def __init__(
        ...
        if os.path.exists(dataset_cache):
            ...
            target = []
            for idx in tqdm(range(len(samples)), position=0, leave=True):
                id_, nxyz, lattice, site_prop, target_val = self.compute_prop(
                    *samples[idx]
                )
                name_list.append(id_)
                nxyz_list.append(nxyz)
    ...
    def compute_prop(self, id_, crystal, target_val):
        site_prop = crystal.site_properties[self.prop_to_predict]
        structure = AA.get_atoms(crystal)
        n = np.asarray(structure.numbers).reshape(-1, 1)
        xyz = np.asarray(structure.positions)
        nxyz = np.concatenate((n, xyz), axis=1)
        ...
        return id_, nxyz, lattice, site_prop, target_val
